...
if [[ "$LINUX_DISTRIBUTION" == $LINUX_DISTRIBUTION_ALPINE ]]; then
	apk add sudo openssl curl wget nano zip htop mc git
	apk update
	apk upgrade
else
	if [[ "$LINUX_DISTRIBUTION" == $LINUX_DISTRIBUTION_DEBIAN ]]; then
		apt-get clean
		apt-get -y install sudo openssl curl wget nano zip htop mc git
		apt-get update
		apt-get -y upgrade
	else
		if [[ "$LINUX_DISTRIBUTION" == $LINUX_DISTRIBUTION_RHEL ]]; then
			...
			dnf -y install sudo openssl curl wget nano zip htop mc git
			dnf update
			dnf upgrade -y
		else
...
- Requires BASH
- Installs utilities like Nano, cURL, Open SSL, SUDo, Zip, HTop, Midnight Commander, etc.
